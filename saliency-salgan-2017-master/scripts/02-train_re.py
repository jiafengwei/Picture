...
def bce_batch_iterator(model, train_data):
    num_epochs = 150
    n_updates = 1
    ...
    for current_epoch in tqdm(range(num_epochs), ncols=20):
        ...
        if current_epoch % 5 == 0:
            np.savez('./' + DIR_TO_SAVE + '/minglanggen_modelWeights{:04d}.npz'.format(current_epoch),
...
def salgan_batch_iterator(model, train_data):
    num_epochs = 150
    nr_batches_train = int(len(train_data) / model.batch_size)
    n_updates = 1
    for current_epoch in tqdm(range(num_epochs), ncols=20):
        ...
        if current_epoch % 3 == 0:
            np.savez('./' + DIR_TO_SAVE + '/regen_modelWeights{:04d}.npz'.format(current_epoch),
                     *lasagne.layers.get_all_param_values(model.net['output']))
            np.savez('./' + DIR_TO_SAVE + '/redisrim_modelWeights{:04d}.npz'.format(current_epoch),
...
def train():
    ...
    with open('/home/s/re/saliency-salgan-2017-master2/minglangtrainData.pickle', 'rb') as f:
